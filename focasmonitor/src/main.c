...
void show_usage(const char *program_name) {
    ...
    printf("Options:\n");
    printf("  --machines=<file>           Load machine list from configuration file\n");
    printf("  --add=<name,ip,port>        Add a machine (can be used multiple times)\n");
    ...
    printf("  --interval=<seconds>        Monitoring interval (default: 30 seconds)\n");
    printf("  --output=<format>           Output format: console, json, csv\n");
    printf("  --verbose                   Enable verbose logging\n");
    printf("  --status                    Show connection pool status\n");
    printf("  --timeout=<seconds>         Connection timeout (default: 10 seconds)\n");
    printf("  --help                      Show this help message\n");
    printf("  --version                   Show version information\n\n");
    printf("Examples:\n");
    printf("  %s --machines=factory.txt --info=basic\n", program_name);
    printf("  %s --add=Mill1,192.168.1.100,8193 --add=Lathe1,192.168.1.101,8193\n", program_name);
...
int read_config(int argc, char *argv[], Config *conf) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strncmp(argv[i], "--machines=", 11) == 0) {
            ...
            strncpy(conf->output_format, argv[i] + 9, sizeof(conf->output_format) - 1);
        } else if (strncmp(argv[i], "--interval=", 11) == 0) {
            conf->monitor_interval = atoi(argv[i] + 11);
            if (conf->monitor_interval < 1) conf->monitor_interval = DEFAULT_MONITOR_INTERVAL;
        } else if (strncmp(argv[i], "--timeout=", 10) == 0) {
            conf->timeout = atoi(argv[i] + 10);
            if (conf->timeout < 1) conf->timeout = CONNECTION_TIMEOUT;
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (result != FOCAS_OK) {
            fprintf(stderr, "Warning: Failed to add machine %s (%s:%d): %s\n", 
                   name, ip, port, focas_result_to_string(result));
            continue;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (g_pool.machine_count == 0) {
        fprintf(stderr, "Error: No machines specified. Use --machines=<file> or --add=<name,ip,port>\n\n");
        show_usage(argv[0]);
        return EXIT_FAILURE;
    }

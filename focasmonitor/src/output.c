...
const char* focas_result_to_string(FocasResult result) {
    switch (result) {
        ...
        case FOCAS_POOL_FULL: return "Connection pool full";
        case FOCAS_MACHINE_NOT_FOUND: return "Machine not found";
        case FOCAS_INVALID_CONFIG: return "Invalid configuration";
...
void print_selective_machine_info(const MachineInfo *info, const char *machine_name, const char *info_type) {
    if (strcmp(info_type, "basic") == 0) {
        printf("%-15s | %-35s | %s\n", machine_name, info->machine_id, info->status);
    } else if (strcmp(info_type, "program") == 0) {
        printf("%-15s | %-10s | N%-8ld | %s\n", 
               machine_name, info->program_name, info->sequence_number, info->status);
    } else if (strcmp(info_type, "position") == 0) {
        printf("%-15s | X:%8.3f | Y:%8.3f | Z:%8.3f | %s\n", 
               machine_name, info->position.x_abs, info->position.y_abs, info->position.z_abs, info->status);
    } else if (strcmp(info_type, "speed") == 0) {
        printf("%-15s | Feed:%5d mm/min | Spindle:%5d RPM | %s\n", 
               machine_name, info->speed.feed_rate, info->speed.spindle_speed, info->status);
    } else if (strcmp(info_type, "alarm") == 0) {
        printf("%-15s | Alarm: %-6s", machine_name, info->alarm.has_alarm ? "ACTIVE" : "NONE");
        if (info->alarm.has_alarm) {
...
void print_machine_info_csv(const MachineInfo *info, const char *machine_name, bool header) {
    if (header) {
        printf("machine_name,machine_id,program_name,program_number,status,sequence_number,");
        printf("x_abs,y_abs,z_abs,x_rel,y_rel,z_rel,feed_rate,spindle_speed,has_alarm,alarm_status,last_updated\n");
    } else {
        printf("%s,%s,%s,%d,%s,%ld,", 
               machine_name, info->machine_id, info->program_name, 
               info->program_number, info->status, info->sequence_number);
        printf("%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,", 
               info->position.x_abs, info->position.y_abs, info->position.z_abs,
               info->position.x_rel, info->position.y_rel, info->position.z_rel);
        printf("%d,%d,%s,%d,%ld\n", 
               info->speed.feed_rate, info->speed.spindle_speed,
               info->alarm.has_alarm ? "true" : "false", 
...
void print_multi_machine_info(const MultiMachineInfo *multi_info, const char *info_type, OutputFormat format) {
    if (format == OUTPUT_JSON) {
        ...
        if (strcmp(info_type, "basic") == 0) {
            ...
            printf("%-15s | %-10s | %-10s | %s\n", "Machine", "Program", "Sequence", "Status");
            printf("%-15s-+-%-10s-+-%-10s-+-%s\n", "---------------", "----------", "----------", "----------");
        } else if (strcmp(info_type, "position") == 0) {
            printf("%-15s | %-10s | %-10s | %-10s | %s\n", "Machine", "X (mm)", "Y (mm)", "Z (mm)", "Status");
            printf("%-15s-+-%-10s-+-%-10s-+-%-10s-+-%s\n", "---------------", "----------", "----------", "----------", "----------");
        } else if (strcmp(info_type, "speed") == 0) {
            printf("%-15s | %-15s | %-15s | %s\n", "Machine", "Feed (mm/min)", "Spindle (RPM)", "Status");
            printf("%-15s-+-%-15s-+-%-15s-+-%s\n", "---------------", "---------------", "---------------", "----------");
        } else if (strcmp(info_type, "alarm") == 0) {
            printf("%-15s | %-12s | %s\n", "Machine", "Alarm Status", "Machine Status");
            printf("%-15s-+-%-12s-+-%s\n", "---------------", "------------", "---------------");
        }
        ...
        for (int i = 0; i < multi_info->machine_count; i++) {
            char machine_name[64];
            snprintf(machine_name, sizeof(machine_name), "Machine_%d", i + 1);
            print_selective_machine_info(&multi_info->machines[i], machine_name, info_type);
        }
        
        printf("\nSummary: %d machines, %d successful reads, %d failed reads\n", 
               multi_info->machine_count, multi_info->successful_reads, multi_info->failed_reads);

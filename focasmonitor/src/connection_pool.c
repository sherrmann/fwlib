...
FocasResult connection_pool_add_machine(ConnectionPool *pool, const char *name, const char *ip, int port) {
...
FocasResult connection_pool_connect_machine(ConnectionPool *pool, int machine_id) {
...
FocasResult connection_pool_disconnect_machine(ConnectionPool *pool, int machine_id) {
    if (!pool || machine_id < 0 || machine_id >= pool->machine_count) {
        return FOCAS_CONNECTION_FAILED;
    }
...
FocasResult read_machine_info(const char *ip, int port, MachineInfo *info) {
    ...
    if (cnc_rdprgnum(libh, &prgnum) == EW_OK) {
        snprintf(info->program_name, sizeof(info->program_name), "O%04d", prgnum.data);
    ...
    if (cnc_statinfo(libh, &status) == EW_OK) {
        switch(status.run) {
            case 0: strcpy(info->status, "STOPPED"); break;
            case 1: strcpy(info->status, "RUNNING"); break;
            case 2: strcpy(info->status, "PAUSED"); break;
            case 3: strcpy(info->status, "ALARM"); break;
            default: 
